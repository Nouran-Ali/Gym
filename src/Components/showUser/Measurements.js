...
const Measurements = () => {
  ...
  const inbody = useMemo(() => inbodies.find((p) => p.id === parseInt(id)), [inbodies, id]);

  const traineeMeasurements = useMemo(() => {
    if (!inbody) return [];
    return inbodies.filter(measurement => measurement.traineeId === inbody.traineeId);
  }, [inbodies, inbody]);

  const sortedMeasurements = useMemo(() => {
    return traineeMeasurements.sort((a, b) => new Date(b.date) - new Date(a.date));
  }, [traineeMeasurements]);
    ...
    <div className='Measurements'>
      <div className='my-2 py-3'>
        <div className='flex items-center justify-between'>
          <h3 className="text-xl font-semibold">{getFormattedDate(new Date(latestMeasurement.date))}</h3>
