...
const Overview = () => {
    ...
    const handleCreateAttendance = async (e) => {
        e.preventDefault();
        const traineeData = { parcode: parcode };
        console.log(traineeData)
        try {
            await dispatch(createAttendance(traineeData)).unwrap();
                ...
                <div>
                    <div className='flex items-center justify-between'>
                        ...
                        <Modal title="تسجيل حضور يدوي" open={isModalOpen} onOk={handleOk} onCancel={handleCancel} footer={null}>
                            <form className='w-3/5 mx-auto mb-10' onSubmit={handleCreateAttendance}>
                                <div className='mb-4'>
                                    <label className='text-[#4E4E4E]'>ID المشترك</label>
                                    <Input className='bg-[#F9F9F9] border border-[#E4E4E4] mt-3' value={parcode} onChange={(e) => setParcode(e.target.value)} />
                                </div>
                                <div className='mt-8'>
                                    <button type='submit' className='bg-[#D9ED4D] rounded-lg py-2 w-full font-semibold'>اضافه</button>
                                </div>
                            </form>
                        </Modal>
                    ...
                    <div className='mt-5 h-lvh overflow-auto'>
                        {(() => {
                            ...
                            return attendance.map((atend, index) => {
